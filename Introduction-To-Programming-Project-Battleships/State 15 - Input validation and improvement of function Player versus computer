...
void initializeShip(char** playerBoard, unsigned boardSize, char shipLetter, char gameMaster) {
    ...
    if (gameMaster == PLAYER) {
        ...
        } while (getDirectionLength > 1 || (direction[0] != 'v' && direction[0] != 'h' && direction[0] != 'V' && direction[0] != 'H'));
    }
    else {
        // Randomly determine direction for computer player
        direction[0] = distribution(gen) % 2 == 0 ? 'v' : 'h';
    }
    ...
    do
    {
        ...
        if (gameMaster == PLAYER) {
            ...
            if (!checkValidCoordinates(playerBoard, boardSize, row, col, shipLetter, direction[0])) {
    ...
    } while (!checkValidCoordinates(playerBoard, boardSize, row, col, shipLetter, direction[0]));


    // Place the ship on the board
    for (int i = 0; i < shipSize; ++i) {
        int curRow = (direction[0] == 'v' || direction[0] == 'V') ? row + i : row;
        int curCol = (direction[0] == 'h' || direction[0] == 'H') ? col + i : col;
